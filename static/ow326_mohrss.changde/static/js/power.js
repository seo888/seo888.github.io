(function () {
    ...
    */
    $(function () {
        ...
        $("#content .freewebinfolist").each(function () {
            var $this = $(this);
            var $prev = $this.prev();
            var prevTagName = $prev.prop("tagName");
            if (prevTagName == "LI" || prevTagName == "TR") {
                $prev  = $prev.parent();
            }
            if ($prev.is(":visible")) {
                $prev.addClass("freewebinfolistarea");
                $prev.attr("data-pos-top", Math.round($prev.offset().top/20))
                $prev.attr("data-pos-left", Math.round($prev.offset().left/20))
            }

        });

        $("#content .freewebloopinfolist").each(function () {
            var $this = $(this);
            var $parent = $this.parent();
            if ($parent.is(":visible")) {
                $parent.addClass("freewebinfolistarea");
                $parent.attr("data-pos-top", Math.round($parent.offset().top/20))
                $parent.attr("data-pos-left", Math.round($parent.offset().left/20))
            }
