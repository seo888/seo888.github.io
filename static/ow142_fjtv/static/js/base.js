...
function hg_h5player_do(obj, index){
	var video = $('#h5player_' + index)[0];
	if (video.paused) 
	{
		video.play();
		$(obj).attr('src', 'http://player.hoge.cn/play1.png');
...
// video上加索引图
var gVideoIndex = 0;
var gLivmediaVideo = new Array();
function hg_video_show(video){
	var tmpvsrc = '';
	if(gLivmediaVideo.length < 1)
	{
    	$('.m2o-livmedia').each(function(i){
    		gLivmediaVideo.push($(this));
    	})
	}
	if (browser.versions.mobile||browser.versions.android||browser.versions.ios)
	{
      var img_info = video.img_info;
      if(img_info)
      {
		  if (video.is_audio == '1')
		  {
                var imgurl1 = img_info.host + img_info.dir + '320x80/' + img_info.filepath + img_info.filename;
                        var playerHtml5 = $('<audio controls></audio>');
			var height = 80;
		  }
		  else
		  {
      	 	var imgurl1 = img_info.host + img_info.dir + '320x240/' + img_info.filepath + img_info.filename;
		  	var playerHtml5 = $('<video controls poster="' + imgurl1 + '"></video>');
			var height = 200;
		  }

	    tmpvsrc = gLivmediaVideo[gVideoIndex].find('video').attr('src');
	    // video播放器
	    
	    playerHtml5.attr({
                id: "h5player_" + gVideoIndex,
	        preload: "none",
	        width: "90%",
	        height: height,
	        src: tmpvsrc
	    }).css({'background':'#000'});  
	    gLivmediaVideo[gVideoIndex].html(playerHtml5);
		
		  if (video.is_audio == '2')
		  {
		var ww = playerHtml5.width();
      	var imgurl = img_info.host + img_info.dir + ww + 'x150/' + img_info.filepath + img_info.filename;
                var playerbtn = '<img onclick="hg_h5player_do(this, ' + gVideoIndex + ')" src="http://player.hoge.cn/play1.png" width="70" style="position:absolute;left:40%;top:30%;z-index:11;">';

                playerHtml5.parent().css({
                    'position': 'relative'
                }).append('<img style="position:absolute;left:50%;margin-left:-' + (ww / 2) + 'px;top:0px;z-index:10;height:150px;" src="' + imgurl + '"/>' + playerbtn);
		  }
      }
	}
	gVideoIndex  = gVideoIndex + 1;
}
...
function commentAdd(obj){	
	...
	if( cval == '' || cval == '文明上网理性发言，请遵守新闻评论服务协议')
	{
	...
	ajaxdata = { 
		cmid : $("#m2odatamap").data("rid"),
		content_id : $("#m2odatamap").data("id"),
		column_id : $("#m2odatamap").data("column_id"),
		content_title : $("#m2odatamap").data("title"),
		content : cval, 
		user_name : author , 
		user_id : uid, 
		...
		app_uniqueid : $("#m2odatamap").data("appuniqueid"),
		mod_uniqueid : $("#m2odatamap").data("moduleuniqueid")
	};

	if($("#app_uniqueid").val() == "column"){
			ajaxdata.app_uniqueid = "column";
			ajaxdata.mod_uniqueid = "column"; 
			ajaxdata.content_id = $("#m2odatamap").data("rid");
	}
	...
	$.ajax({
		...
		success:function(data){
			//console.log('1.',actionurl,'2.',ajaxdata,'3.',data);
			if(data.ErrorCode ){
				commentInfo({"obj" : obj,"message" : "评论失败," + data.ErrorText ,"type" : "error"});				
			}
			else{
				data = data[0];
				if(data.error){
		...
		success:function(data){					
			if(!data){
				...
				$.each(data.data,function(index, item){
					...
					item.avatar = item.avatar ? item.avatar : "http://www.fjtv.net/t/1/14/images/user_fj.png";
...
function emotionInit(){
		var emotion_mood ,	
			emotion_total = 0, 
			emotion_percent = 0, 
			emotion_id = $("#emotion").attr('data-cmid');
