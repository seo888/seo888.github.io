...
var mbd_config = {
    ...
    homePage : {
        // 首页地址可能有多个，多个用";"隔开
        pageUrl : "http://web.sxdaily.com.cn/" 
    },
    column: {
        // 判断是否为栏目页的正则表达式
        iscurrent: /^((https|http):\/\/)?(www\.sxdaily\.com\.cn){1}\/node_\d+.html$/, 
        ...
        // 判断是否为文章页的正则表达式
        iscurrent: /^((https|http):\/\/)?(www\.sxdaily\.com\.cn){1}\/([\s\S]+)\/content_\d+.html$/, 
    ...
    getColumnName : function(){
        ...
        if(mbd_config.column.common){
            ...
            var tagElement = mbd_utils.getElesByClassName("div", "local_nav");
            if(typeof tagElement=='undefined' || tagElement.length==0){
                tagElement = mbd_utils.getElesByClassName("div", "localnav");
            }
            ...
            if(typeof tagElement!='undefined' && tagElement.length>=1){
                ...
                if(typeof aEles!='undefined' && aEles.length>=1){
                    for(i=0;i<aEles.length;i++){
                        ...
                        if(i==0 && (curName=='首页' || curName=='主页' || curName=='陕西传媒网')) continue;
